...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $sql = "UPDATE `crud` SET `name`='".$name."',`email`='".$email."',`mobile`='".$mobile."',`password`='".$password."' WHERE id = ".$_SESSION['id']."";
    ...
    <form method="post" action="<?php echo ($_SERVER['PHP_SELF']);?>"> 
        <div class=" lg:w-2/3 w-full  mx-auto px-8 items-end">
         <div class="relative flex-grow w-full">
          ...
          <input type="name" id="name" name="name" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-transparent focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
        </div>
        <div class="relative flex-grow w-full">
        ...
        <div class="relative flex-grow w-full">
          ...
          <input type="mobile" id="mobile" name="mobile" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-transparent focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
        </div>
        <div class="relative flex-grow w-full">
        ...
        <button name="sumit" class="text-white bg-indigo-500 border-0 py-2 my-7 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">update</button>
      </div>
      </form>
    </div>
  </section>
